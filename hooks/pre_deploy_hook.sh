...
# run bst tests to possibly block ask deploy with broken code
run_bst_tests() {
    echo "in run_bst_tests()"
    bst test
    return $?
}
# end run bst tests
...
# run bst test
if run_bst_tests; then
    echo "Tests passed."
else
    echo "Tests failed. Open test_output/report/index.html" \
            "to see results." &>/dev/tty
    exit 1
fi
# end run bst test
